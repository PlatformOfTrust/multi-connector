*** Settings ***
Documentation     LVI-Info.fi API Tests
Library           Collections
Library           DateTime
Library           PoTLib
...
${ID1}                       56501228138
&{BROKER_BODY_PARAMETERS}    codeProductCatalog=${ID1}
...
fetch, 200
    ...
    Fetch Data Product    ${body}
    Integer               response status                                         200
    String                response body @context                                  https://standards.oftrust.net/v2/Context/DataProductOutput/ProductCatalog/
    Object                response body data
    Object                response body data product
    String                response body data product codeProductCatalog           ${ID1}
...
fetch, 422, Missing data for ids required field
    [Tags]                 bug-0003
    ${body}                Get Body
    Pop From Dictionary    ${body["parameters"]}                codeProductCatalog
    Fetch Data Product     ${body}
    Integer    response status                                  422
    Integer    response body error status                       422
    String     response body error message parameters.codeProductCatalog 0     Missing data for required field.

fetch, 200, Empty ids
    [Tags]                 bug-0004
    ${body}                Get Body
    Set To Dictionary      ${body["parameters"]}                codeProductCatalog=@{EMPTY}
    Fetch Data Product     ${body}
    Integer    response status                200
    String     response body @context         https://standards.oftrust.net/v2/Context/DataProductOutput/ProductCatalog/
